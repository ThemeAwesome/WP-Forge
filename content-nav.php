...
<a class="skip-link screen-reader-text" href="#content"><?php _e( 'Skip to content', 'wp-forge' ); ?></a>
    ...
    <?php if( get_theme_mod( 'wpforge_nav_position' ) == 'top') { ?>
        ...
        <?php } // end if ?>
            <nav class="top-bar" data-topbar data-options="scrolltop:false; mobile_show_parent_link: true">
                ...
                <section class="top-bar-section">
                    <?php
                    $link_position = esc_attr(get_theme_mod ('wpforge_link_position','right'));
                    wp_nav_menu( array(
                        'theme_location' => 'primary',
                        'container' => false,
                        'depth' => 0,
                        'items_wrap' => '<ul class="'. $link_position .'">%3$s</ul>',
    ...
    <?php if( get_theme_mod( 'wpforge_nav_position' ) == 'fixed') { ?>
        ...
        <?php if( get_theme_mod( 'wpforge_mobile_display' ) == 'yes') { ?>
        <div class="fixed hide-for-small hide-for-medium">
        <?php } else { ?>
        <div class="fixed row">
        <?php } // end if ?>
            <nav class="top-bar" data-topbar data-options="scrolltop:false; mobile_show_parent_link: true;">
